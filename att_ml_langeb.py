...
class Attention:
    def __init__(self, model, training_src, training_tgt):
        ...
        self.dec_builder = dy.GRUBuilder(self.layers, self.embed_size+self.contextsize, self.hidden_size*2, self.model)

        self.W_y = self.model.add_parameters((self.tgt_vocab_size, self.hidden_size*2+self.contextsize))
        self.b_y = self.model.add_parameters(self.tgt_vocab_size)

        self.attention_size = 128
        self.lang_eb_size = 10
        self.langeb_lookup = self.model.add_lookup_parameters((len(self.lang_li), self.lang_eb_size))
    ...
    def step_batch(self, batch):
        ...
        langeb = dy.lookup_batch(self.langeb_lookup, [self.lang_li.index(self.rsrc_vocab[item]) for item in src_sent[1]])
...
def main(argv):
    ...
    epoch_num = 30
    train_data = zip(attention.change_word2id(training_src, attention.src_vocab), attention.change_word2id(training_tgt, attention.tgt_vocab))
    del training_src
    del training_tgt
    ...
    for i in xrange(epoch_num):
        ...
        for j, end in enumerate(dev_ends):
            ...
            print 'Epoch', i + 1, sequence[j], 'Valid', bleuscore
            start = end
        ...
        for j, end in enumerate(test_ends):
            ...
            print 'Epoch', i + 1, sequence[j], 'Test', test_bleu
            start = end
