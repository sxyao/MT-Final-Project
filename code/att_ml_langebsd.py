...
class Attention:
    def __init__(self, model, training_src, training_tgt, lang_li):
        ...
        self.src_vocab, self.rsrc_vocab = self.change_word2id_genevoc(training_src)
        ...
        self.langeb_lookup = self.model.add_lookup_parameters((len(lang_li), self.lang_eb_size))
        ...
        self.W1_att_lang = self.model.add_parameters((self.attention_size, self.lang_eb_size))
        self.w2_att = self.model.add_parameters((1, self.attention_size))

        self.max_len = 50

    def __attention_mlp_batch(self, H_f_batch, h_e_batch, W1_att_e, W1_att_f, w2_att, W1_att_lang, langeb):
        # H_f_batch: (2 * hidden_size, num_step, batch_size)
        # h_e_batch: (hidden_size, batch_size)
        a_t_batch = dy.tanh(dy.colwise_add(dy.colwise_add(W1_att_f * H_f_batch, W1_att_e * h_e_batch), W1_att_lang * langeb)) # (attention_size, num_step, batch_size)
    ...
    def __attention_mlp(self, H_f, h_e, W1_att_e, W1_att_f, w2_att, W1_att_lang, langeb):

        # Calculate the alignment score vector
        a_t = dy.tanh(dy.colwise_add(dy.colwise_add(W1_att_f * H_f, W1_att_e * h_e), W1_att_lang * langeb))
        a_t = w2_att * a_t
        a_t = a_t[0]
    ...
    def step_batch(self, batch, lang):
        ...
        langid = self.lang_li.index(lang)
        langeb = dy.lookup_batch(self.langeb_lookup, [langid]*batch_size)
        ...
        for (cw, nw) in zip(tgt_sent[0:-1], tgt_sent[1:]):
            ...
            c_t = self.__attention_mlp_batch(h_fs_matrix, h_e, W1_att_e, W1_att_f, w2_att, W1_att_lang, langeb)
    ...
    def translate_sentence(self, sent, lang):
        ...
        while len(trans_sentence) < self.max_len:
            ...
            c_t = self.__attention_mlp(h_fs_matrix, h_e, W1_att_e, W1_att_f, w2_att, W1_att_lang, langeb)
    ...
    def evaluate(self, dev_src, dev_tgt, outpath, lang):
        hypos = []
        reference = []
        fout = codecs.open(outpath, 'w', 'utf-8')
        for i in xrange(len(dev_src)):
            sen_src = dev_src[i]
            p_sen = self.translate_sentence(sen_src, lang)
    ...
    def translate_corpus(self, src, outpath, lang):
        fout = codecs.open(outpath,'w','utf-8')
        for sen_src in src:
            p_sen = self.translate_sentence(sen_src, lang)
            fout.write(u' '.join(p_sen)+u'\n')
    ...
    def set_dropout(self, p):
        ...
        for ele in self.lang_li:
            self.dec_builder[ele].set_dropout(p)

    def disable_dropout(self):
        self.l2r_builder.disable_dropout()
        self.r2l_builder.disable_dropout()
        for ele in self.lang_li:
            self.dec_builder[ele].disable_dropout()
...
def main(argvs):
    ...
    for k in lang_li:
        alltraining_src += argvs[k][1]
    training_tgt = {}
    for k in lang_li:
        training_tgt[k] = argvs[k][2]

    attention = Attention(model, alltraining_src, training_tgt, lang_li)
    del alltraining_src
    # training_src = argv[1]
    # training_tgt = argv[2]
    # dev_src = argv[3]
    ...
    for k in argvs:
        train_data[k] = zip(attention.change_word2id(argvs[k][1],attention.src_vocab),
                            attention.change_word2id(argvs[k][2],attention.tgt_vocab[k]))
        train_data[k] = divide_batch(train_data[k], batch_size)
        total[k] = len(train_data[k])
    maxtotal = max(total.values())

    dev_src = {}
    test_src = {}
    for k in argvs:
        dev_src[k] = attention.change_word2id(argvs[k][3], attention.src_vocab)
        test_src[k] = attention.change_word2id(argvs[k][5], attention.src_vocab)
    ...
    for i in xrange(epoch_num):
        ...
        while startidx < maxtotal:
            for k in train_data:
                for bid in xrange(startidx, min(startidx+1000,total[k])):
                    batch = train_data[k][bid]
                    losses, num_words = attention.step_batch(batch,k)
                    if count[k] % 50 == 0:
                        tem = losses.value()
                        print 'Iter', k, count[k], '/', total[k], tem / (num_words * len(batch))
                    losses.backward()
                    trainer.update()
                    count[k] += 1
            startidx+=1000
        ...
        for k in lang_li:
            bleuscore = attention.evaluate(dev_src[k], argvs[k][4], 'grudp_output/' + argvs[k][7] + '_' + str(i), k)
            print 'Epoch', i + 1, k, 'Valid', bleuscore

            test_bleu = attention.evaluate(test_src[k], argvs[k][6], 'grudp_output/' + argvs[k][8] + '_' + str(i), k)
            print 'Epoch', i + 1, k, 'Test', test_bleu
...
def test_separate_decoder():
    parser = argparse.ArgumentParser()
    parser.add_argument('-src', type=str, default='en')
    parser.add_argument('-tgts', type=str, default='es,fr')
    parser.add_argument('--dynet-mem', type=int, default=5000)
    parser.add_argument('--dynet-gpu-ids', type=int)
    args = parser.parse_args()
    langli = args.tgts.split(',')
    ...
    for i in xrange(len(langli)):
        if langli[i] in fulls:
            langli[i] = fulls[langli[i]]

    for i in xrange(len(langli)):
        spl = splits[(args.src, langli[i])]
        training_src = read_file(spl[args.src]['train'])
        training_tgt = read_file(spl[langli[i]]['train'])
        dev_src = read_file(spl[args.src]['valid'])
        dev_tgt = read_file(spl[langli[i]]['valid'])
        test_src = read_file(spl[args.src]['test'])
        test_tgt = read_file(spl[langli[i]]['test'])
        dev_out = spl[langli[i]]['valid'].split('/')[-1]
        test_out = spl[langli[i]]['test'].split('/')[-1]
        argv = ['',
